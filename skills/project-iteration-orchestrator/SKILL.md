...
- `tdd-development`: may be delegated for independent test-first slices when `software_design.md` is clear, behavior can be tested on the host, and the ECR allows implementation under explicit write boundaries. Prefer it for new feature behavior, design-driven implementation, complex changes that benefit from tests before code, or explicit user requests for TDD. The main agent still owns slice selection, architecture, shared-state sequencing, final integration, and acceptance reporting.
- `cubemx-code-implementation`: may be delegated only for independent module driver deltas, direct non-TDD implementation, build/flash-oriented implementation work, or independent implementation review. Business logic, HMI flow, scheduling, startup behavior, callback integration, and final application integration remain main agent responsibilities.
...
5. Main agent chooses the implementation path:
   - use `tdd-development` directly or dispatch eligible independent TDD slices when the design document defines testable behavior, the change is a new feature or complex behavior, host-side tests are practical, or the user explicitly asks for TDD
   - use `cubemx-code-implementation` directly or dispatch eligible work for direct implementation, build integration, flashing, non-TDD changes, or CubeMX/HAL-heavy work where host tests are not the primary path
